...
for i = 1:7
    ...
    n = numel(images(:,:,1));
    double_images = zeros(size(images,1), size(images,2), size(images,3));
    
    D_im = [];
    
    %     for j = 1: size(images,3)
    %         temp = im2double(images(:,:,j));
    %         double_images(:,:,j) = temp / norm(temp);
    %     end
...
for i = 1: 7
    ...
    overall_cov{i} = overall_cov{i} + (alpha * I);
    for j = 1:10
        cov_X{i}{j} = cov_X{i}{j} + (alpha * I);
        Y_a(:, j) = mvnpdf(double(T_im_vect), mean_X{i}{j}, overall_cov{i});
        Y_b(:, j) = mvnpdf(double(T_im_vect), mean_X{i}{j}, cov_X{i}{j});
    end
    
    for r = 1: size(Y_a,1)
        [probability, idx_a] = max(Y_a(r,:));
        [probability, idx_b] = max(Y_b(r,:));
        if idx_a -1 - T_labels(r) == 0
            count_a = count_a + 1;
        end
        if idx_b - 1 - T_labels(r) == 0
            count_b = count_b + 1;
        end
        
    end
    
    error_a(i) =  1 -   double(count_a) / double(size(T_labels, 1))
    error_b(i) =  1 -   double(count_b) / double(size(T_labels, 1))
